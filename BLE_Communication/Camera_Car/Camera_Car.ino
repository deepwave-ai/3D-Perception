...
void disableNotifications() {
    for (auto characteristic : characteristics) {
        characteristic->setNotifyProperty(false); // Disable notifications
    }
    Serial.println("Notifications disabled for all characteristics");
}

void stopBLE() {
    if (pServer) {
        pServer->disconnect(0); // Disconnect all clients
        delay(100);             // Allow time for connections to terminate
    ...
    if (pService) {
        ...
        try {
            ...
            Serial.println("Exception occurred while stopping BLE service");
        }
    } else {
...
void stopAndDisableBLE() {
    ...
    // Forcefully disable BLE stack
    Serial.println("Forcing BLE stack deinitialization...");
    BLEDevice::deinit(true);
    Serial.println("BLE stack fully disabled.");
}
